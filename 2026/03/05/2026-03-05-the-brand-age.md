# The Brand Age

- Score: 189 | [HN](https://news.ycombinator.com/item?id=47264756) | Link: https://paulgraham.com/brandage.html

### TL;DR

Paul Graham uses the Swiss watch industry to argue that technology commoditizes substantive product differences, leaving brand as the profitable differentiator. Japanese competition, the Swiss franc’s rise, and quartz accuracy crushed mechanical-watch sales; survivors such as Patek Philippe shifted from thinness and precision toward recognizable designs, advertising, artificial scarcity, and status. Revenue recovered even as functional superiority vanished. Graham concludes that branding often conflicts with convergent good design and advises following interesting problems instead. Commenters agreed exclusivity signals access, but disputed that luxury watches are inherently ugly or poorly designed.

### Comment pulse

- Artificial scarcity upgrades wealth into access → waiting lists, prior purchases, and retailer relationships create a story money alone cannot buy.
- Aesthetic judgment split sharply → critics called oversized branded forms compromised — counterpoint: owners and admirers see deliberate, beautiful design.
- Brand value extends beyond pure luxury → Apple users may pay partly for shared cultural positioning while still valuing product advantages.

### LLM perspective

- **View:** Functional convergence does not end design competition; it redirects design toward recognizable social signals and stories.
- **Impact:** Incumbents can preserve margins through scarcity, while technically strong entrants must first manufacture cultural legibility.
- **Watch next:** Production volumes, resale premiums, advertising spend, waitlist access, and whether independent brands sustain differentiation.
